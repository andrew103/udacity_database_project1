...
def popular_article():
    ...
    # use slug to relate IP views to articles: where log.path = '%'articles.slug
    c.execute("""select articles.title, articles.author, cast(count(articles.slug) as int) as num
                    from articles, log
...
def popular_author():
    ...
    # use double select statement with inner being the select statement from the popular_article function
    c.execute("""select authors.name, cast(sum(article_views.num) as int) as num
                    from authors, (select articles.title, articles.author, cast(count(articles.slug) as int) as num
                                    from articles, log
                                    where log.path like concat('%', articles.slug)
                                    group by articles.title, articles.author
...
def percent_error_requests():
    ...
    # find a way to pull status messages given the same day then do math for avg
    c.execute("""select a.date, concat(cast((b.num * 100.0/(a.num + b.num))as float), '%')
